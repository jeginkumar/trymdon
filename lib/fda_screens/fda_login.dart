...
import 'package:trymd_partner/fda_screens/verify_fda_otp.dart';
...
class LoginForFDA extends StatefulWidget {
  final userType;
  LoginForFDA({this.userType});

  @override
  _LoginForFDAState createState() => _LoginForFDAState();
}

class _LoginForFDAState extends State<LoginForFDA> {
  ...
  gotoLogin() async {
    var response = await AuthenticationsForFDA()
        .getLoginResponseFDA(mobileNumberController.text, widget.userType);
    if (response['status'] == 1) {
      Navigator.pushReplacement(
        context,
        PageRouteBuilder(
          pageBuilder: (context, animation1, animation2) => OtpVerificationFDA(
      ...
      _globalWidget.toastMessage('Invalid mobile number');
    }
  }
}
