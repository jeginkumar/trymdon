...
class VendorClassificationGet {
  ...
  Future getClassification() async {
    final response = await http
        .get(Uri.parse(AppConfig().baseUrl + "/getvendorClassification"));
    Map<String, dynamic> responseMap = json.decode(
      utf8.decode(response.bodyBytes),
    );
    return responseMap;
  ...
  // get vedor names
  Future getVendorNames() async {
    final response =
        await http.get(Uri.parse(AppConfig().baseUrl + "/getvendorName"));
    Map<String, dynamic> responseMap = json.decode(
      utf8.decode(response.bodyBytes),
    );
    return responseMap;
  ...
  // Update preferences - category , specialty ,  labels , services
  ...
  Future updateOperationDetails(jsonBody) async {
    final response = await http.post(
        Uri.parse(AppConfig().baseUrl + "/postOperationalsettime"),
        body: jsonBody,
        headers: {'Content-type': 'application/json'});
    Map<String, dynamic> responseMap = json.decode(
      utf8.decode(response.bodyBytes),
